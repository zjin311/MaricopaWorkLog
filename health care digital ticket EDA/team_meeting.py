...
filename='2-2-2021 dircks update Facility Itemized Tracker (1)'
...
#%%
df_both['year']=df_both['release date'].dt.year.astype('str')
df_both['month']=df_both['release date'].dt.month.astype('str')
df_both['week']=df_both['release date'].dt.week.astype('str')
#%%
df_both['yearweek']=df_both['year']+'_'+df_both['week']
df_both['category']=df_both['category'].replace({'ventilator ':'ventilator','test':'testing supplies','t':'testing supplies','hand sanitizer':'sanitizer','disinfectant':'sanitizer','thermometers':'thermometer','gowns':'gown','other items':'other'})
['2021_16','2021_28','2021_51','2021_53']
df_both=df_both[~df_both['yearweek'].isin(['2021_16','2021_28','2021_51','2021_53'])]
#df_both=df_both[df_both.index.notin([368, 11614, 11616, 11617, 11619, 15463, 16072])]
#df_both.to_csv('Tableau viz Total PPE2.csv')
#%%
def lineplot_by_yearweek():
    
    fig, ax1, = plt.subplots()
    fig.set_size_inches(25, 12)
    sns.barplot(x='yearweek',y='qty out',\
        data=df_both,ax=ax1,estimator=sum)
    plt.xticks(fontsize=18,rotation=90)
    plt.yticks(fontsize=18)
    plt.xlabel('yearweek', fontsize=30)
    plt.ylabel('sum', fontsize=25)
    plt.title('Total Giving Trend', fontsize=30)
    plt.legend(loc=2, prop={'size': 20})
    plt.tight_layout() 
    save_fig('PPE total giveout Monthly trending')
    plt.show()
lineplot_by_yearweek()

#%%
df_total_out_both_alltime=df_both.groupby('category')['qty out'].sum().sort_values(ascending=False).reset_index()
#%%
def barplot_by_year(df):
    
    fig, ax1, = plt.subplots()
    fig.set_size_inches(60, 25)
    sns.barplot(x='category',y='qty out',\
        data=df,ax=ax1)
    ...
    plt.tight_layout() 
    save_fig(' total amount given by category'.format(datt))
    plt.show()
barplot_by_year(df_total_out_both_alltime)
# %%

...
filename='9 29 2020 Facility Itemized Tracker (1)'
...
df_smt['received date']=pd.to_datetime(df_smt['received date'])
#%%
'''df total within a certain time range'''


#df_smt['release date'].apply(lambda x:x=df_smt['received date'])

def PPE_by_dateRange(date1,date2):

    df_by_date=df_smt[(df_smt['release date']>=date1) & (df_smt['release date']<=date2)]
    return df_by_date.groupby('category')['qty out'].sum().sort_values(ascending=False)
df_by_date1=PPE_by_dateRange('2020-11-09','2020-11-15')
print(df_by_date)
df_by_date.to_excel('2020-11-09----2020-11-15 total give out.xlsx')


df_by_date2=PPE_by_dateRange('2020-11-16','2020-11-19')
print(df_by_date2)
df_by_date2.to_excel('2020-11-11----2020-11-16 total give out.xlsx')



#%%
'''
# product source project!


df_source=df_smt.groupby('project')['qty out'].sum().reset_index()
df_source.columns=['Product Source','Total Amount']
source_t=pd.concat([df_source,resource_df])
source_t=source_t.groupby('Product Source')['Total Amount'].sum()
source_t=source_t.reset_index()
'''
#%%

...
filename='6_10 update Facility Itemized Tracker(1)'
...
#%%
drop_ems_april(df_clean)
#%%


                                    '''EDA String manipulate'''

def str_manipulate(df_clean_EMS):
...
#check one facility type's total amount
df_clean_EMS[df_clean_EMS['Facility Type']=='Ltcf'].groupby('Type')['Total Amount'].sum()
#%%
#check all
df_clean_EMS.groupby(['Facility Name','Type'])['Facility Type','Total Amount'].sum()
...
#%%
'''
12 weeks average PPE Burn rate without EMS and Acute
...
# we have some type of product which has only 1 record so that the[1] is out of range for that category
'''
type_unique=df_clean_EMS.Type.unique()
maxnumber=[]
for i in type_unique:
...
weekly_mean_by_type=[]
latest_week=max(df_clean_EMS['week_of_year'].unique())
oldest_week=min(df_clean_EMS['week_of_year'].unique())

week_span=int(latest_week)-int(oldest_week)


for i in type_unique:
    weekly_mean_by_type.append(int(df_clean_EMS[df_clean_EMS['Type']==i]['Total Amount'].sum()/week_span))
...
df_ana['weekly_mean']=weekly_mean_by_type
print(all_time_mean_by_type)
...
std_by_type=[]
for i in type_unique:
    std_by_type.append(np.std(df_clean_EMS[df_clean_EMS['Type']==i].groupby('week_of_year')['Total Amount'].sum()))
...
#%%
# 1---6 std
df_ana['78 confident order amount']=df_ana['std']+df_ana['weekly_mean']
...
df_ana['4std confident order amount']=df_ana['std']*4+df_ana['weekly_mean']
df_ana['5std confident order amount']=df_ana['std']*5+df_ana['weekly_mean']
df_ana['6std confident order amount']=df_ana['std']*6+df_ana['weekly_mean']

#%%
#10 week total amount to order
df_ana['10 week order (all time high)']=df_ana['All_Time_High_Weekly']*10
df_ana['10 week order (78 confident)']=df_ana['78 confident order amount']*10
...
df_ana['10 week order (4std confident)']=df_ana['4std confident order amount']*10

df_ana['10 week order (5std confident)']=df_ana['5std confident order amount']*10

df_ana['10 week order (6std confident)']=df_ana['6std confident order amount']*10

#%%
df_ana.to_csv('6_11 updated PPE Order amount prediction with confidence level by zixiang michael Jin.csv')
...
# %%
'''Project : GIS connection'''
path='C:/Users/Mr.Goldss/Desktop/health care digital ticket EDA/'
filename='Hospital'
Gis_hospital=pd.read_excel(path+filename+'.xlsx')


# %%
hospital_name=Gis_hospital.LEGALNAME.unique()
print(hospital_name)

# %%
hospital_facility_name=df_clean_EMS[df_clean_EMS['Facility Type']=='Acute']['Facility Name'].unique()

# %%

import fuzzywuzzy
from fuzzywuzzy import process
for name in hospital_facility_name:
    matches=process.extract(name,hospital_name,limit=len(hospital_name))
    for potential_match in matches:
        if potential_match[1]>=80:
            df_clean_EMS['Facility Name changed']=potential_match[0]


# %%
print(process.extract('Mayo Hospital',hospital_name,limit=len(hospital_name)))

# %%
df_gis=df_clean_EMS.groupby(['Facility Name','Type'])['Facility Type','Total Amount'].agg(['first','sum'])
df_gis.to_csv('All type Facility and total given.csv')

# %%
df_clean_EMS[df_clean_EMS['Facility Name']=='AZ State Hospital'].groupby(['Facility Name','Type'])['Total Amount'].sum()
